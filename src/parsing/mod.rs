...
impl PropertyGraphParser {
    ...
    fn extract_properties(&self, v : Pair<'_, Rule>, props : &mut HashMap<String, String>) {
        match v.as_rule() {
            Rule::propertySpec => v.into_inner().for_each(|i| self.extract_properties(i, props)),
            Rule::properties => v.into_inner().for_each(|i| self.extract_properties(i, props)),
            Rule::property => {
    ...
    fn extract_label_and_props(&self, v : Pair<'_, Rule>, labels : &mut Vec<String>, props: &mut HashMap<String, String>) -> bool {
        match v.as_rule() {
    ...
    fn handle_result(&self, v : Pair<'_, Rule>, graph : &mut PropertyGraph, names : &mut HashMap<String, NodeIndex>) {
        match v.as_rule() {
            Rule::createGraphType => v.into_inner().for_each(|i| self.handle_result(i, graph, names)),
            Rule::graphType => v.into_inner().for_each(|i| self.handle_result(i, graph, names)),
            Rule::graphTypeElements => v.into_inner().for_each(|i| self.handle_result(i, graph, names)),
            Rule::elementTypes => v.into_inner().for_each(|i| self.handle_result(i, graph, names)),
            Rule::elementType => v.into_inner().for_each(|i| self.handle_result(i, graph, names)),
            Rule::nodeType => {
                let mut pairs = v.into_inner();
                ...
                let labels : Vec<_> = labels.into_iter().map(|label| graph.vertex_label.add_label(label)).collect();
                for label in labels {
            ...
            Rule::edgeType => {
                let mut pairs = v.into_inner();
                let first_name = pairs.next().unwrap().into_inner().next().unwrap().as_str().to_string();
                ...
                let last_name = pairs.next().unwrap().into_inner().next().unwrap().as_str().to_string();
                ...
                let labels : Vec<_> = labels.into_iter().map(|label| graph.edge_label.add_label(label)).collect();
                for label in labels {
